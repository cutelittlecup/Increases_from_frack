        ...
        if col == '#2':
            if self.tree.identify_region(event.x, event.y) != 'heading':
                def ok(event):
                    ...
                    entry.destroy()
                    try:
                        if self.indexes[0] == 19:
                            objects_info[self.objects][self.indexes[row]] = [float(self.tree.set(item)['Значение'])]
                        else:
                            objects_info[self.objects][self.indexes[row]] = float(self.tree.set(item)['Значение'])

                        if self.indexes[0] == 24:
                            objects_info_for_graphs[self.objects][1][row][0] = float(self.tree.set(item)['Значение'])
                        if self.indexes[0] == 28:
    ...
    class Info_for_listbox:
        ...
        def choose_form(self, event):
            ...
            for i in range(len(params)):
                ...
                try:
                    to_tree2 = [params[i], round(sum(self.objects_info[self.choose_params][i + 19])/
                                                 len(self.objects_info[self.choose_params][i + 19]), 2)]
        ...
        for i in range(len(keys)):
            ...
            if objects_info[keys[i]][39] < 4:
                ...
                if type(objects_info[keys[i]][24]) == float:
                    ...
                    print('plan_Xf: ' + str(plan_Xf))
                    # Mpr_stock = 0
                    try:
                        fact_Xf = a_Xf * math.log(Mpr_stock) + b_Xf                                            # ФАКТ Xf
                    except ValueError:
                        fact_Xf = 0
                    print('fact_Xf: ' + str(fact_Xf))
                    ...
                    print('Пластовое давление')
                    print(objects_info[keys[i]][21])
                    print('Забойное давление')
                    print(objects_info[keys[i]][22])
                    print('Обводнённость')
                    ...
                    viscosity_pot = viscosity_liq_finder(B_tr / 100, viscosity_oil, viscosity_water, B0)
                    ...
                    if formula_type == 1:
                        ...
                        try:
                            ...
                            Q_water = (K_good * K * (P_pl_tr - P_zab_tr) * nnt) / (viscosity_start * B0 * (
                                    math.log((a_dgoshi + math.sqrt(a_dgoshi * a_dgoshi - (l_fact / 2) ** 2)) / (l_fact / 2))
                                    + coeff_anizotropii ** 0.5 * nnt / l_fact * math.log(
                                coeff_anizotropii ** 0.5 * nnt / (coeff_anizotropii ** 0.5 + 1) / rw) + 0) * 18.41)
                            print('                                            ПОСЧИТАЛОСЬ')
                        ...
                        try:
                            ...
                            if P_pl_2_months < P_nas:
                                K_dupui = Q_w * 18.4 * B0 * viscosity_start * (math.log(Re / rw) - 0.75 + S_nns) / (
                            ...
                            """
                            if P_pl_tr < P_nas:
                                Q_water_dupui = K_dupui * nnt / (18.4 * viscosity_pot * B0 * (math.log(Re / rw) - 0.75 + skin)) * (
    ...
    for i in range(len(stock_TR_array)):
        ...
        if scepka_TR[i] in date_keys:
            ...
            if (len(df) != 0) and ('РАБ.' in xr_df_array or 'НАК.' in xr_df_array or 'РАБ' in xr_df_array or 'НАК' in xr_df_array):
                ...
                for j in range(len(dates)):
                    ...
                    if k == 0 and j == len(dates) - 1:
                        try:
                            ...
                            df = df
                array_to_check = df['забойное давление (тр), атм'].tolist()
                t = 0
                per_cent = 0.8
                iteration = 0
                number = 0
                for d in range(len(array_to_check)):
                    if array_to_check[d] != 0:
                        number = d
                        break

                while t == 0:
                    for p in range(number, len(array_to_check)):
                        if (array_to_check[number] - array_to_check[p])/array_to_check[number] > per_cent - iteration > 0:
                            df = df.iloc[p]
                            t = 1
                            break
                        elif (array_to_check[number] - array_to_check[p])/array_to_check[number] > per_cent - iteration \
                                and per_cent - iteration < 0 and t == 0:
                            df = df.iloc[number]
                            t = 1
                            break
                ...
                info[scepka_TR[i]] = [float(df['дебит жидкости (тр), м3/сут']),
                                      float(df['обводненность (тр), % (объём)']),
                                      float(df['пластовое давление (тр), атм']),
                                      float(df['забойное давление (тр), атм']),
                                      float(df['дебит нефти (тр), т/сут']),
