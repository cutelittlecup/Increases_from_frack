...
def main(array_with_file_names):
        ...
        def choose_stock(self, event):
            ...
            if len(self.objects_dates_for_graphs[self.choose_params]) > 1:
                ...
                d = plot3.plot([x_array3[len(x_array) - 1] + h, x_array3[len(x_array) - 1] + h],
                               [0, objects_info[self.choose_params][36]], color='brown', label='  Дебит нефти',
                               linewidth=14)
                e = plot3.plot([x_array3[len(x_array) - 1] + h, x_array3[len(x_array) - 1] + h],
                               [objects_info[self.choose_params][34],
                                objects_info[self.choose_params][34] + objects_info[self.choose_params][33]],
    ...
    def formulas(objects_info, keys):
        ...
        for i in range(len(keys)):
            ...
            if type(n_grp_povt) == int:

                if n_grp_povt < 4:

                    if type(objects_info[keys[i]][23]) == float and objects_info[keys[i]][2] != 0:
                        ...
                        #print('nnt: ' + str(nnt))
                        print([keys[i]])
                        if objects_info[keys[i]][0] == "ГС" or 1:
                            print('n: ' + str(n_mgrp_fact))
                            if n_mgrp_fact > 1:
                        ...
                        while objects_info[keys[i]][31] < Q_water and iterator <= len(objects_info[keys[i]][18]):
                            ...
                            #print('Пластовое давление за ' + str(iterator) + ' месяцев')
                            #print(P_pl_for_k)
                            #print('Забойное давление за ' + str(iterator) + ' месяцев')
                            #print(P_zab_for_k)
                            #print('Обводнённость за ' + str(iterator) + ' месяцев')
                            #print(B_for_k)
                        ...
                        try:
                            ...
                            objects_info[keys[i]][36] = 0

                    else:
                        objects_info[keys[i]][33] = ''
                        objects_info[keys[i]][34] = ''
                        objects_info[keys[i]][35] = ''
    ...
    TR = TR.dropna(axis=0, subset=['объекты работы', 'пластовое давление (тр), атм', 'обводненность (тр), % (объём)',
                                   'дебит нефти (тр), т/сут', 'дебит жидкости (тр), м3/сут', 'забойное давление (тр), атм'])
        ...
        if keys_new_stock_Frack[i] in refracks_keys:
            if refracks[keys_new_stock_Frack[i]][0] != '-':
                ...
                if len(min_array) != 0:
                    ...
                    Mpr_last[keys_new_stock_Frack[i]] = sum(Mpr_Frack_new[keys_new_stock_Frack[i]][idx_min])/\
                                                        len(Mpr_Frack_new[keys_new_stock_Frack[i]][idx_min])
                else:
    ...
    for i in range(len(keys)):
        for j in range(4):
            objects_info[keys[i]].append(0)                                                                 # Кпрон [33]
                                                                                                             # Qжид [34]
        objects_info[keys[i]].append('')
