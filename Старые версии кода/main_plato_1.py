    ...
    class Info_for_listbox:
        ...
        def choose_form(self, event):
            ...
            for i in range(len(params)):
                try:
                    to_tree1 = [params[i], round(self.objects_info[self.choose_params][i + 18], 2)]
            ...
            if len(self.objects_dates_for_graphs[self.choose_params]) > 1:
                ...
                print(self.objects_dates_for_graphs[self.choose_params][1][0])
                print(x_array)
                plot1.minorticks_on()
                ...
                x_array3 = x_array
                #x_array3.append(x_array3[len(x_array3) - 1] + h)

                plot1.set_xlim(x_array[0] - h/2, x_array[len(x_array) - 1] + h)
                plot2.set_xlim([x_array[0] - h/2, x_array[len(x_array) - 1] + h])

                d = plot3.plot([x_array3[len(x_array) - 1] + h, x_array3[len(x_array) - 1] + h],
                               [0, objects_info[self.choose_params][35]], color='brown', label='  Дебит нефти',
                               linewidth=14)
                e = plot3.plot([x_array3[len(x_array) - 1] + h, x_array3[len(x_array) - 1] + h],
                               [objects_info[self.choose_params][35],
                                objects_info[self.choose_params][35] + objects_info[self.choose_params][33]],
                ...
                plot1.legend(myl, labs, loc='upper left', fontsize=8)

                plot1.set_xlabel('Дата', fontsize=10)
                plot1.set_ylabel('Qж (по МЭР), м3/сут', fontsize=10)
                plot2.set_ylabel('Обводнённость (по МЭР), %', fontsize=10)
                plot3.set_ylabel('Qж (расч.), м3/сут', fontsize=10)
        ...
        for i in range(len(keys)):
            ...
            if n_grp_povt < 4:
                ...
                if type(objects_info[keys[i]][23]) == float:
                    ...
                    P_pl_2_months = objects_info[keys[i]][20]                             # Pпл ЗАПУСКНЫЕ (2 мес)
                    P_zab_2_months = objects_info[keys[i]][21]                      # Pзаб ЗАПУСКНЫЕ (2 мес)
                    B_2_months = objects_info[keys[i]][19]                                # % ЗАПУСКНЫЕ (2 мес)
                    Q_w_2_months = objects_info[keys[i]][18]                               # Qж ЗАПУСКНЫЕ (2 мес)
            ...
            if grp_povt[i] < 4:
                ...
                if type(objects_info[keys[i]][23]) == float:
                    ...
                    try:
                        ...
                        K_pron_for_excel.append(objects_info[keys[i]][32])
                        Q_water_for_excel.append(objects_info[keys[i]][33])
                    ...
                    P_pl_start_for_excel = round(objects_info[keys[i]][20], 2)
                ...
                while t == 0:
                    for p in range(number, len(array_to_check)):
                        if (array_to_check[number] - array_to_check[p]) / array_to_check[number] > per_cent - iteration > 0:
                            ...
                            t = 1
                            break
                        elif (array_to_check[number] - array_to_check[p]) / array_to_check[number] > per_cent - iteration and per_cent - iteration < 0 and t == 0:
                            df = df.iloc[number]
                            t = 1
                            break
                    iteration += 0.1
