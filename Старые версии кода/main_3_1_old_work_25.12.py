...
def main(array_with_file_names):
            ...
            if n_grp_povt < 4:
                ...
                if Mpr_stock == 0 and objects_info[keys[i]][1] != 0:
                    ...
                    if keys[i] not in list(objects_info_problems.keys()):
                        objects_info_problems[keys[i]] = 'Нет данных по массе пропанта в файле ФРАК'

                if type(objects_info[keys[i]][23]) == float and (for_djoshi != 0):
                    #print('')
    ...
    TR = TR.dropna(axis=0, subset=['объекты работы', 'пластовое давление (тр), атм', 'обводненность (тр), % (объём)',
                                   'дебит нефти (тр), т/сут', 'дебит жидкости (тр), м3/сут',
    ...
    for i in range(len(form_New_strat)):
        ...
        scepka_to_New_strat.append(str(field_New_strat[i]) + '_' + str(form_New_strat[i]) + '_' + str(stock_New_strat[i]))
        if str(field_New_strat[i]) + '_' + form_New_strat[i] + '_' + str(stock_New_strat[i]) not in scepka_New_strat and str(field_New_strat[i]) + '_' + form_New_strat[i] + '_' + str(stock_New_strat[i]) in keys:
            scepka_New_strat.append(str(field_New_strat[i]) + '_' + str(form_New_strat[i]) + '_' + str(stock_New_strat[i]))
    ...
    for i in range(len(keys)):
        if keys[i] in grp_first_keys:
            ...
            objects_info[keys[i]].append(0)
            if (keys[i] not in date_Frack_new_keys) and (keys[i] not in list(objects_info_problems.keys())):
    ...
    for i in range(len(scepka_TR)):
        ...
        if (len(df) != 0) and ('РАБ.' in xr_df_array or 'РАБ' in xr_df_array or 'НАК.' in xr_df_array or 'НАК' in xr_df_array):
            ...
            info[scepka_TR[i]] = [[], [], [], [], [], '', '', '', '', '', '', '', 'Скважина в бездействующем фонде', '']
